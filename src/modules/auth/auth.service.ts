...
export class AuthService {
	...
	async register(registerDto: RegisterDto) {
		...
		if (isExists) throw new BadRequestException(this.i18.t('auth.exists'));
	...
	async refresh(refreshToken: string) {
		if (!refreshToken)
			throw new UnauthorizedException(this.i18.t('auth.refreshNotProvided'));
		const result = await this.jwtService.verifyAsync(refreshToken);

		if (!result)
			throw new UnauthorizedException(this.i18.t('auth.refreshTokenIncorrect'));
	...
	private async validate(loginDto: LoginDto) {
		...
		if (user?.lockedUntil >= new Date())
			throw new BadRequestException(this.i18.t('auth.blocked'));

		if (!user) {
			throw new UnauthorizedException(this.i18.t('auth.validate'));
		}
